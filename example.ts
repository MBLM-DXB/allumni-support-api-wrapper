...
async function desk365Example() {
  ...
  try {
    ...
    if (userTickets.tickets.length > 0) {
      ...
      const escalatedTicket = await client.escalateTicket(ticketId, TicketPriority.HIGH);
 ...
 */
class TicketService {
  private supportApi: SupportApiInterface;
  
  constructor(supportApi: SupportApiInterface) {
  ...
  async respondToTicket(data: TicketResponseRequest) {
   ...
   */
  async respondToTicketWithAttachments(data: TicketResponseRequest, attachments: Array<string | File>) {
    // If the underlying provider supports attachments directly
    if ('respondToTicketWithAttachments' in this.supportApi) {
      return (this.supportApi as any).respondToTicketWithAttachments(data, attachments);
    }
  ...
  async addNoteWithAttachments(
    ...
    if ('addNoteWithAttachmentsToTicket' in this.supportApi) {
      return (this.supportApi as any).addNoteWithAttachmentsToTicket(
        ticketId,
        note,
        attachments,
        isPrivate,
    ...
    if ('addNoteToTicket' in this.supportApi) {
      return (this.supportApi as any).addNoteToTicket(
        ticketId,
        note,
        isPrivate,
        agentEmail,
  ...
  async assignTicket(ticketId: string, agentEmail: string) {
    return this.supportApi.assignTicket(ticketId, agentEmail);
  }
  
  async escalateTicket(ticketId: string, priority: TicketPriority) {
    return this.supportApi.escalateTicket(ticketId, priority);
  }
  
  async updateTicket(data: UpdateTicketRequest) {
...
async function providerAgnosticExample() {
  ...
  const desk365Client = new Desk365Client({
    baseUrl: 'https://your-subdomain.desk365.io/apis',
    apiKey: process.env.DESK365_API_KEY || 'your-api-key'
  });
      ...
      if (typeof window === 'undefined') {
        try {
          ...
          if (fs.existsSync(filePath)) {
            ...
            await ticketService.addNoteWithAttachments(
              ticketId,
              'This is a private note with an attachment',
              [filePath],
              true,
              'agent@example.com'
            );
 ...
 */
export async function ticketApiExample(req: any, res: any) {
  const { method } = req;
  
  // Create the support API client
  const supportApi = new Desk365Client({
    ...
    switch (method) {
      case 'GET':
        ...
        if (ticketId) {
          ...
          const ticket = await supportApi.getTicketDetails(ticketId as string);
          return res.status(200).json(ticket);
        } else if (email) {
          // List tickets for a user
          const tickets = await supportApi.listUserTickets(email as string);
