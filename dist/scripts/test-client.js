...
const LOG_FILE = path_1.default.join(LOG_DIRECTORY, `desk365-test-${new Date().toISOString().replace(/:/g, '-')}.log`);
if (LOG_TO_FILE) {
    if (!fs_1.default.existsSync(LOG_DIRECTORY)) {
        fs_1.default.mkdirSync(LOG_DIRECTORY);
    }
 ...
 */
function logErrorToFile(prefix, error) {
    var _a, _b, _c, _d, _e, _f, _g;
    if (!LOG_TO_FILE)
        return;
    try {
        const logStream = fs_1.default.createWriteStream(LOG_FILE, { flags: 'a' });
        logStream.write(`\n========== ${prefix} at ${new Date().toISOString()} ==========\n`);
        if ((0, axios_1.isAxiosError)(error)) {
            logStream.write(`Status: ${(_a = error.response) === null || _a === void 0 ? void 0 : _a.status} ${(_b = error.response) === null || _b === void 0 ? void 0 : _b.statusText}\n`);
            logStream.write(`URL: ${(_c = error.config) === null || _c === void 0 ? void 0 : _c.url}\n`);
            logStream.write(`Method: ${(_e = (_d = error.config) === null || _d === void 0 ? void 0 : _d.method) === null || _e === void 0 ? void 0 : _e.toUpperCase()}\n`);
            if ((_f = error.response) === null || _f === void 0 ? void 0 : _f.headers) {
            ...
            logStream.write(JSON.stringify((_g = error.response) === null || _g === void 0 ? void 0 : _g.data, null, 2) + '\n');
        }
        else {
            logStream.write((error === null || error === void 0 ? void 0 : error.stack) || String(error) || 'Unknown error');
            logStream.write('\n');
        }
        logStream.end();
    }
...
function formatError(error) {
    ...
    if ((0, axios_1.isAxiosError)(error)) {
        ...
        const allowedMethods = ((_d = (_c = error.response) === null || _c === void 0 ? void 0 : _c.headers) === null || _d === void 0 ? void 0 : _d.allow) || '';
        let message = '';
        ...
        if (message)
            result += `: ${message}`;
        if (allowedMethods)
            result += ` (Allowed methods: ${allowedMethods})`;
        return result;
    }
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                const TEST_FILE_PATH = path_1.default.join(LOG_DIRECTORY, 'test-attachment.txt');
                if (!fs_1.default.existsSync(TEST_FILE_PATH)) {
                    fs_1.default.writeFileSync(TEST_FILE_PATH, 'This is a test attachment created for API testing purposes.');
                ...
                try {
                    ...
                    const noteWithAttachment = await client.addNoteWithAttachmentsToTicket(ticketId, 'This is a test private note with an attachment.', [TEST_FILE_PATH], true, // isPrivate
            ...
            try {
                await client.escalateTicket(ticketId, types_1.TicketPriority.HIGH);
